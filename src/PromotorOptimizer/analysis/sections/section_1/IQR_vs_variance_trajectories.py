...
def simulate_trajectory_paths(
    ...
    iteration states from the configuration cohorts.
    """
    df_working = df.sort_values(['interpreter', 'optimizer', 'model_type', 'sequence_name', 'iteration']).copy()
        ...
        for path_idx in range(num_simulated_paths):
            ...
            for it in unique_iters:
                ...
                if iter_candidates.empty:
                    continue
                ### Sample a single candidate row per iteration to form a trajectory line
